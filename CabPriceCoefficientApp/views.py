...
from CabPriceCoefficientApp.serializers.output_serializer import CabPriceCoefficientSerializer
from CabPriceCoefficientApp.serializers.input_serializer import LatitudeLongitudeSerializer
...
class CabPriceCoefficientView(APIView):
    ...
    def __init__(self):
        super().__init__()
        self.serializer_class = LatitudeLongitudeSerializer
        self.redis_dao = RedisDataBaseAccessObject()

    def post(self, request) -> Response:
        """
           Cabs price coefficient generated by 'lat' (latitude), 'lon' (longitude) of cabs location

        """

        serialized_latitude_longitude = LatitudeLongitudeSerializer(data=request.data)
        if serialized_latitude_longitude.is_valid():
            latitude_and_longitude_dict: Dict = serialized_latitude_longitude.data
            geolocator = Nominatim(user_agent="snapp_cab_app")
            address_of_the_cab: Dict = \
                geolocator.reverse((latitude_and_longitude_dict['latitude'], latitude_and_longitude_dict['longitude']),
                                   language='en', exactly_one=True).raw['address']
